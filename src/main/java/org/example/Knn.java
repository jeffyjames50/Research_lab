...
public class Knn {
    public void runKnn(DataModel datamodel) throws IOException {
        ...
        findAverageMAE(datamodel,pip);
        findAverageMAE(datamodel,jmsd);
        findAverageMAE(datamodel,cosine);
        findAverageMAE(datamodel,correlation);
        findAverageMAE(datamodel,singularity);



    }

    private static void findAverageMAE(DataModel datamodel,UserSimilarityMetric metric){
        UserKNN userKNN;
        userKNN = new UserKNN(datamodel, 75,metric, UserKNN.AggregationApproach.WEIGHTED_MEAN);
        userKNN.fit();
