...
import es.upm.etsisi.cf4j.recommender.neural.NCCF;
...
public class NCF {
    ...
    public void runNcf(DataModel datamodel) throws IOException {
        NCCF ncf;


        ncf = new NCCF(datamodel,10);
...
//            }
            ...
            for (String ageGroup : ageGroups.keySet()) {
                ...
                if (age >= range[0] && age <= range[1]) {
                    double[] predictions = ncf.predict(testUser);
                    double mae = new MAE(ncf).getScore(testUser, predictions);
